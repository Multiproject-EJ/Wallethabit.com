...
export default function Account() {
  ...
  const lockedModules = modules.filter((module) =>
    !unlockedModules.some((unlock) => unlock.module.id === module.id),
  )
    ...
    <div className="flex flex-1 flex-col gap-8">
      <header className="space-y-4">
        ...
        <p className="max-w-3xl text-sm text-slate-600">
          Manage the customer record that lives in Supabase. Purchases, module access, and plan data update instantly so
          members always see the experiences they have unlocked.
        </p>
      </header>
      ...
      {session ? (
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-600 shadow-sm">
          ...
          <p className="mt-2">
            Head to the <Link to="/auth" className="font-semibold text-brand hover:text-brand-dark">auth portal</Link>{' '}
